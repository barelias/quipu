...
describe('joinText', () => {
  ...
  it('returns empty string for null', () => {
    expect(joinText(null as unknown as undefined)).toBe('');
  });
});
...
describe('joinSource', () => {
  ...
  it('returns empty string for null/undefined', () => {
    expect(joinSource(null as unknown as string)).toBe('');
    expect(joinSource(undefined as unknown as string)).toBe('');
  });
});

describe('inferLanguage', () => {
  it('reads from kernelspec', () => {
    const nb = { metadata: { kernelspec: { language: 'python' } } } as Parameters<typeof inferLanguage>[0];
    expect(inferLanguage(nb)).toBe('python');
  });

  it('reads from language_info when kernelspec absent', () => {
    const nb = { metadata: { language_info: { name: 'julia' } } } as Parameters<typeof inferLanguage>[0];
    expect(inferLanguage(nb)).toBe('julia');
  });

  it('falls back to python', () => {
    expect(inferLanguage({} as Parameters<typeof inferLanguage>[0])).toBe('python');
