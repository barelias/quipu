...
describe('kernelService — browser runtime', () => {
  let kernelService: Awaited<typeof import('../services/kernelService')>['default'];
  let isElectron: Awaited<typeof import('../services/kernelService')>['isElectron'];

  beforeEach(async () => {
    // Ensure Electron APIs are absent so browser impl is selected
    delete window.electronAPI;
    vi.resetModules();
    const mod = await import('../services/kernelService');
