...
const FindBar: React.FC<FindBarProps> = ({ editor, onClose }) => {
  ...
  const syncStorage = useCallback(() => {
    if (!editor) return;
    setTotalMatches((editor.storage as Record<string, any>).findReplace?.totalMatches ?? 0);
    setCurrentIndex((editor.storage as Record<string, any>).findReplace?.currentIndex ?? -1);
  }, [editor]);
