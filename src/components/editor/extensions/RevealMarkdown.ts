 ...
 */
function getMarkRange($pos: ResolvedPos, markType: MarkType): MarkRange | null {
  const { parent } = $pos;
 ...
 */
function createSyntaxWidget(text: string, extraClass: string = ''): HTMLSpanElement {
        ...
        props: {
          decorations(state) {
            ...
            if (parentTypeName === 'heading') {
              const level = (parentNode.attrs.level as number) || 1;
            ...
            if (parentTypeName === 'horizontalRule' || (
              !empty && (state.selection as any).node?.type.name === 'horizontalRule'
            )) {
            ...
            if (parentTypeName === 'codeBlock') {
              const lang = (parentNode.attrs.language as string) || '';
              const startPos = $from.before($from.depth);
