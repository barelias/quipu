...
const Terminal = forwardRef<TerminalHandle, TerminalProps>(({ workspacePath }, ref) => {
  const {
    terminalTabs,
    activeTerminalId,
    createTerminalTab,
  ...
  const electronHandlerRef = useRef<(() => void) | null>(null);
  ...
  const writeToInstance = useCallback((instance: TerminalInstance, data: string) => {
  ...
  const createXtermInstance = useCallback((_tabId: string): TerminalInstance => {
  ...
  const connectTerminal = useCallback(async (_tabId: string, instance: TerminalInstance) => {
    const { xterm } = instance;

    if (isElectron()) {
      // Electron: create pty via service, data comes through shared IPC channel
      try {
        const result = await terminalService.create(workspacePath ?? undefined);
        ...
        showToast('Failed to create terminal: ' + (err as Error).message, 'error');
      }
    } else {
      ...
      const connect = () => {
        ...
        dataHandler = xterm.onData((data: string) => {
  ...
  useEffect(() => {
    ...
    const handler = terminalService.onData(({ terminalId, data }: { terminalId: string; data: string }) => {
    ...
    electronHandlerRef.current = handler as (() => void) | null;

    return () => {
      if (handler) {
        terminalService.removeDataListener(handler as () => void);
  ...
  useEffect(() => {
    ...
    for (const [tabId, instance] of instancesRef.current) {
      if (!terminalTabs.find((t: TerminalTab) => t.id === tabId)) {
        // Kill backend
        if (isElectron() && instance.backendId) {
    ...
    >
      ...
      <div className="flex items-center bg-[#1e1e1e] border-b border-white/[0.08] shrink-0">
        <div className="flex items-center flex-1 overflow-x-auto min-w-0">
          {terminalTabs.map((tab: TerminalTab) => (
            <button
              key={tab.id}
