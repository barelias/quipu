...
describe('Terminal WebSocket Reconnection Logic', () => {
  ...
  beforeEach(() => {
    ...
    let dataHandler: { dispose: any } | null = null;
    ...
    const connect = () => {
      ...
      dataHandler = mockXterm.onData((data: string) => {
  ...
  it('stopReconnect prevents further retries', () => {
    ...
    const reconnectCalls = mockXterm.writeln.mock.calls.filter(
      (c: string[]) => c[0].includes('Reconnecting')
    );
  ...
  it('intentional close does not trigger reconnect', () => {
    ...
    const reconnectCalls = mockXterm.writeln.mock.calls.filter(
      (c: string[]) => c[0].includes('Reconnecting') || c[0].includes('Disconnected')
    );
