/**
 * Domain service for repos — load all, save one, delete one, plus folder
 * operations. Built on the file-store primitives in `quipuFileStore` and
 * the slug helpers in `slug`.
 *
 * Layout assumption: `<workspacePath>/.quipu/repos/<folder>/<slug>.json`.
 * - Multi-level folders are real on-disk directories.
 * - An entity's id is its folder-relative slug-path (`external/foo`),
 ...
 *
 * Near-twin of `agentFileStore`. The folder logic is identical; only the
 * record shape and the on-disk root differ. Don't extract a shared base
 * until a third store appears.
 */
 ...
 * marker file; folders that contain at least one repo are implicit
 * (a real directory exists on disk).
 */
export interface FolderNode {
  /** Forward-slash separated path relative to .quipu/repos/. Empty for the root (rare). */
  path: string;
...
/** Compute the on-disk root for repos under a workspace. */
function reposRoot(workspacePath: string): string {
  return joinAbs(workspacePath, '.quipu', 'repos');
}

/** Given a folder-relative id (`external/foo`), build the absolute file path. */
function repoFilePath(workspacePath: string, id: string): string {
  return `${joinAbs(reposRoot(workspacePath), id)}.json`;
}
...
function folderAbsPath(workspacePath: string, folderPath: string): string {
  const root = reposRoot(workspacePath);
 ...
 * an entity id. `external/foo.json` -> `external/foo`.
 */
...
/**
 * Given a record loaded from disk, normalize it into a Repo:
 ...
 * or `url`, for example) — callers translate those throws into "skip + warn".
 */
function normalizeLoaded(raw: unknown, relativePath: string): Repo {
  if (raw === null || typeof raw !== 'object') {
    throw new Error('Repo file is not an object');
  }
  const record = raw as Partial<Repo> & { id?: unknown; slug?: unknown; folder?: unknown };
  const id = relPathToId(relativePath);
  const { folder, slug } = splitId(id);
  if (typeof record.name !== 'string') {
    throw new Error('Repo file is missing required "name" field');
  }
  if (typeof record.url !== 'string') {
    throw new Error('Repo file is missing required "url" field');
  }
  return {
    ...record,
    id,
    ...
    name: record.name,
    url: record.url,
    localClonePath: typeof record.localClonePath === 'string' ? record.localClonePath : undefined,
    createdAt: typeof record.createdAt === 'string' ? record.createdAt : new Date(0).toISOString(),
    updatedAt: typeof record.updatedAt === 'string' ? record.updatedAt : new Date(0).toISOString(),
  } as Repo;
}

/** Strip derived fields before serializing a repo to disk. */
function toDiskRecord(repo: Repo, slug: string, folder: string): Record<string, unknown> {
...
/**
 * Load every repo file under `<workspacePath>/.quipu/repos/`.
 *
 ...
 *
 * Returns repos sorted by id for deterministic ordering.
 */
export async function loadAllRepos(workspacePath: string): Promise<Repo[]> {
  const root = reposRoot(workspacePath);
  ...
  for (const entry of entries) {
    try {
      ...
      repos.push(normalizeLoaded(raw, entry.relativePath));
    } catch (err) {
      // One bad file shouldn't take down the whole workspace.
      console.warn('[repos] failed to load', entry.relativePath, err);
    }
  }
  repos.sort((a, b) => (a.id < b.id ? -1 : a.id > b.id ? 1 : 0));
  return repos;
}
 ...
 * implicit ancestors of any repo file.
 *
 * Returned folders are deduped by path and sorted by path. The root
 * folder (path `''`) is NOT included even if `.quipu/repos/.folder.json`
...
export async function loadAllFolders(workspacePath: string): Promise<FolderNode[]> {
  ...
  // 2. Implicit ancestors of every repo file.
  const entries = await listJsonFilesRecursive(root);
  for (const entry of entries) {
...
/**
 * Save `repo` to disk. The repo's id is recomputed from its current
 ...
 * Returns the new id.
 *
 * Caller is responsible for normalizing `repo.folder` (via
 * `normalizeFolder`) and slugifying `repo.slug` (via `slugify` +
...
export async function saveRepo(
  ...
  if (slug === '') {
    throw new Error('saveRepo: repo.slug is required');
  }
  const newId = joinId(folder, slug);
  const newAbs = repoFilePath(workspacePath, newId);
  const record = toDiskRecord(repo, slug, folder);
  ...
  if (previousId !== undefined && previousId !== newId) {
    const oldAbs = repoFilePath(workspacePath, previousId);
...
/**
 * Delete the repo's JSON file. Idempotent — missing files are not an
 * error.
 */
export async function deleteRepo(workspacePath: string, id: string): Promise<void> {
  const abs = repoFilePath(workspacePath, id);
 ...
 * Delete a folder.
 *
 * - With `recursive: true`, all children (repos and subdirectories) are
 *   removed in one shot.
 * - With `recursive: false` (default), child repos are moved into the
 *   parent folder (or root) preserving slugs, with disambiguation if the
 *   destination already has a sibling using the same slug. The folder
 *   itself is then removed (it's empty by then).
 */
export async function deleteFolder(
  ...
  // Find every repo file currently under this folder.
  ...
  if (entries.length > 0) {
    ...
    const allRepos = await loadAllRepos(workspacePath);
    const destSiblings = new Set(
      allRepos
        .filter((r) => (r.folder ?? '') === destFolder)
        // Skip repos that live inside the folder being collapsed —
        // they're about to move out, so they aren't really siblings.
        .filter((r) => !idIsInsideFolder(r.id, folderPath))
        .map((r) => r.slug ?? splitId(r.id).slug),
    );
    ...
    for (const entry of entries) {
      ...
      let repo: Repo;
      try {
        repo = normalizeLoaded(raw, entry.relativePath);
      } catch (err) {
        console.warn('[repos] deleteFolder: failed to load child', entry.relativePath, err);
        continue;
      }
      repo.folder = destFolder;
      repo.slug = newSlug;
      await saveRepo(workspacePath, repo, oldFullId);
    }
  }
