...
// __excalidrawProps is declared in the excalidraw mock (src/test/__mocks__/excalidraw.tsx)

const makeProps = (content: string, filePath: string, onContentChange?: (content: string) => void) => ({
  tab: { path: filePath, name: filePath.split('/').pop()! } as import('@/types/tab').Tab,
  activeFile: { content, name: filePath.split('/').pop()!, path: filePath, isQuipu: false } as import('@/types/tab').ActiveFile,
  onContentChange,
});
