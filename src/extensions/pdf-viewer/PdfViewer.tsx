...
const ANNOTATION_TYPES: PdfAnnotationType[] = [
  "comment",
  "review",
  "todo",
  "bug",
...
const PdfViewer: React.FC<PdfViewerProps> = ({ tab }) => {
  ...
  const [commentButtonPos, setCommentButtonPos] = useState<ButtonPosition>({ top: 0, left: 0 });
  ...
  const fileUrl = useMemo<string>(() => fs.getFileUrl(filePath), [filePath]);
  ...
  const loadComments = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      if (frame && frame.annotations) {
        setComments(
          frame.annotations
            .filter((a: FrameAnnotation) => a.page != null)
            .map((a: FrameAnnotation): PdfComment => ({
              id: a.id,
              page: a.page!,
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      const commentsByPage: Record<number, PdfComment[]> = {};
      for (const c of visibleComments) {
        if (!commentsByPage[c.page]) commentsByPage[c.page] = [];
        commentsByPage[c.page].push(c);
      }

      for (const [pageNumStr, pageComments] of Object.entries(commentsByPage) as [string, PdfComment[]][]) {
        ...
        pageComments.forEach((comment) => {
          ...
          for (const span of spans) {
            ...
            try {
              ...
              const startOffset = Math.min(idx, (textNode as Text).length);
  ...
  const addComment = useCallback(async (): Promise<void> => {
    ...
    const annotation: Omit<PdfComment, "id" | "timestamp"> = {
  ...
  const updateCommentType = useCallback(
    async (commentId: string, newType: PdfAnnotationType): Promise<void> => {
      setComments((prev) =>
                      ...
                      : (pageRefs.current[c.page]?.offsetTop || 0) +
                        (c.topRatio || 0) *
                          ((pageRefs.current[c.page]?.querySelector(
                            ".react-pdf__Page",
                          ) as HTMLElement | null)?.offsetHeight || ESTIMATED_PAGE_HEIGHT),
              ...
              >
                <div className="flex justify-between mb-1 text-xs">
                  <select
                    ...
                    onChange={(e) => updateCommentType(c.id, e.target.value as PdfAnnotationType)}
